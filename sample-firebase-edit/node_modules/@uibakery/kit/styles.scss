@mixin alignment-styles() {
  ...
  .slot.slot-direction-row > * {
    margin: 0 0.5rem;
  }

  .slot.slot-direction-column > * {
    margin: 0.5rem 0;
  }
  ...
  nb-card {
    ...
    @each $status in nb-get-statuses() {
      ...
      &.background-#{$status} {
        ...
        nb-card-header {
          border-bottom-color: nb-theme(color-#{$status}-600);
        }

        nb-card-footer {
          border-top-color: nb-theme(color-#{$status}-600);
        }
      }
    }
    ...
    &.background-disabled {
      ...
      nb-card-header {
        border-bottom-color: nb-theme(card-disabled-header-border);
      }

      nb-card-footer {
        border-top-color: nb-theme(card-disabled-footer-border);
      }
    }
    ...
    &.background-hint {
      ...
      nb-card-header {
        border-bottom-color: nb-theme(card-hint-header-border);
      }

      nb-card-footer {
        border-top-color: nb-theme(card-hint-footer-border);
      }
    }
...
@mixin slot-styles() {
  ...
  // all slots, excluding tabs and accordions
  .slot:not(.space):not(nb-tab):not(nb-accordion-item) {
    @include slot();
  }

  // only slot of an active tab
  nb-tab.slot.content-active[class] {
    @include slot();
  }

  // we don't need card header and footer to grow
  nb-card-header.slot, nb-card-footer.slot {
    height: auto;
  }

  // accordion has slot class on some parent element, but styles
  // should be applied deeper
  nb-accordion-item.slot.expanded nb-accordion-item-body .item-body {
    @include slot();
  }
}
...
@mixin space-styles() {
  ...
  .space {
    box-shadow: 0 0 0 1px var(--dev-ui-space-border-color);
